  ...
  return (
    ...
    >
      ...
      <ShopfrontPanel
        ...
        selectedOfferSlotId={selectedOfferSlot?.inventoryItemId}
        onOfferSlot={setSelectedOfferSlot}
      />
...
function OfferPanel({
  ...
  farm: SocialFarmCard;
  selectedSlot: ShopDisplaySlotView | null;
}) {
  ...
  return (
    <Panel tone="sky">
      ...
      {!selectedSlot ? (
        <Text style={styles.bodyText}>Tap a produce card in the shopfront above to start an offer.</Text>
      ) : (
        <View style={styles.offerForm}>
          <View style={styles.selectedOfferItem}>
            <ProduceImage slot={selectedSlot} size={46} />
            <View style={styles.cardTitleBlock}>
              <Text style={styles.itemName}>{selectedSlot.item.name}</Text>
              <Text style={styles.cardMeta}>Selected from shopfront</Text>
            </View>
          </View>
...
function ShopfrontPanel({
  ...
  onOfferSlot?: (slot: ShopDisplaySlotView) => void;
        ...
        {visibleSlots.length ? (
          visibleSlots.map((slot) => (
            <ShelfCard
              ...
              onOfferPress={onOfferSlot ? () => onOfferSlot(slot) : undefined}
              offerSelected={selectedOfferSlotId === slot.inventoryItemId}
            />
          ))
        ) : (
...
function ShelfCard({
  ...
  const cardStyle = [styles.shelfCard, compact && styles.shelfCardCompact, offerSelected && styles.shelfCardSelected];

  if (onOfferPress) {
    return (
      <Pressable
        accessibilityRole="button"
        accessibilityState={{ selected: offerSelected }}
