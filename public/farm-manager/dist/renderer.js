...
export function project(point, height = 0) {
...
export function unproject(screenX, screenY) {
...
function projectedBoundaryBounds(zoom = 1) {
...
export function getCenteredPan(zoom = state.zoom) {
...
function drawCropField(object) {
    ...
    drawCropRows(object);
    if (isOverCapacity(object))
        drawCapacityWarning(object);
    if (state.selectedId === object.id)
        drawSelection(object);
}
...
function drawTerrainPatches() {
    ...
    for (let x = bbox.minX; x < bbox.maxX; x += patchW) {
        for (let y = bbox.minY; y < bbox.maxY; y += patchH) {
            ...
            ], 0.02, colors[Math.abs(Math.floor(x * 7 + y * 11)) % colors.length], "transparent");
        }
    }
}
...
function drawSelectedEditHandles(objects) {
    if (state.mode !== "edit" && state.mode !== "move")
        return;
    const object = objects.find((item) => item.id === state.selectedId);
    if (object)
        drawEditHandles(object);
}
...
function drawDraft() {
    if (state.mode !== "draw")
        return;
    if (!state.draft.length && !state.mouse)
        return;
...
function generatePositions(poly, count, seed, offset) {
    ...
    while (positions.length < count && guard < 500) {
        ...
        if (G.pointInPolygon(point, poly))
...
export function hitTestAll(world) {
...
function isOverCapacity(object) {
    if (object.type !== "cropField" && object.type !== "livestock")
        return false;
    const idealSpace = Number(object.attrs.idealSpaceSqft);
    const count = Number(object.attrs.count);
    if (!Number.isFinite(idealSpace) || idealSpace <= 0 || !Number.isFinite(count) || count <= 0)
        return false;
