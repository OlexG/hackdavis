...
export const DEFAULT_FARM_BOUNDS = [
    [0, 0],
    [108, 0],
    [108, 82],
    [0, 82]
];
export function cropArea(id, label, polygon, attrs = {}) {
    return {
        id,
        label,
        type: "cropArea",
...
export function cropField(id, label, polygon, parentId, cropKey, attrs = {}) {
    const crop = cropKey
...
export function livestock(id, label, polygon, attrs) {
    return {
        id,
        label,
        type: "livestock",
...
export function structure(id, label, polygon, attrs) {
    return {
        id,
        label,
        type: "structure",
...
export function path(id, label, points, attrs = {}) {
    return {
        id,
        label,
        type: "path",
...
export function currentObjects() {
    return state.objects;
}
export function setCurrentObjects(objects) {
...
export function createCommit(name) {
    const now = new Date();
...
export function resetForManualPlan() {
    const now = new Date();
    state.objects = [];
...
function geoPolygonToLocal(points) {
    if (!points || points.length < 3)
        return clone(DEFAULT_FARM_BOUNDS);
...
export function deleteObject(id) {
    ...
    if (!object)
        return;
    const childIds = object.type === "cropArea"
        ? state.objects.filter((item) => item.type === "cropField" && item.parentId === object.id).map((item) => item.id)
        : [];
...
export function loadCommit(index) {
...
export function useAiPreset() {
    const preset = clone(initialObjects);
    preset.push(cropArea("south-rotation", "South Rotation", [[45, 62], [65, 59], [73, 74], [50, 78]], {
